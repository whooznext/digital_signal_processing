...
hn=np.logical_and(n>=0, n<=M).astype(float) / (M+1)
...
X=np.sum(xn*(np.cos(W*n)-1j*np.sin(W*n)), axis=1)
H=np.sum(hn*(np.cos(W*n)-1j*np.sin(W*n)), axis=1)
Y=np.sum(yn*(np.cos(W*nn)-1j*np.sin(W*nn)), axis=1)
